@echo off
REM ============================================================
REM AMPNM Windows Monitoring Agent (Legacy Single-Run Version)
REM ============================================================
REM NOTE: This is the legacy single-run version.
REM For better monitoring, use the new agents in assets/windows-agent/
REM   - AMPNM-Agent-Installer.ps1 (Windows Service - recommended)
REM   - AMPNM-Agent-Simple.bat (Loop-based batch script)
REM ============================================================

set SERVER_URL=http://localhost:2266/api.php?action=submit_metrics
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference = 'SilentlyContinue'; ^
   ...
   $payload = @{ host_name=$env:COMPUTERNAME; host_ip=$ip; cpu_percent=[math]::Round($cpu,2); memory_percent=$memPercent; memory_total_gb=$totalMemGB; memory_free_gb=$freeMemGB; disk_total_gb=$diskTotal; disk_free_gb=$diskFree; network_in_mbps=$inMbps; network_out_mbps=$outMbps; gpu_percent=$gpuAvg }; ^
   try { ^
